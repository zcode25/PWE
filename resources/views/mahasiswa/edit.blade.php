...
@section('container')
  ...
  <div class="row">
    <div class="col-xl-6">
      <form action="{{ route("update", ['nim' => $mahasiswa->nim ]) }}" method="POST">
        @method('PUT')
        @csrf
        <div class="mb-3">
          <label for="nim" class="form-label">NIM</label>
          <input type="number" class="form-control @error('nim') is-invalid @enderror" id="nim" name="nim" value="{{ old("nim", $mahasiswa->nim)}}">
          @error('nim')
        ...
        <div class="mb-3">
          ...
          <select class="form-select" id="jenis_kelamin" name="jenis_kelamin">
            @foreach ($kategori as $kategori)
                @if (old('jenis_kelamin', $mahasiswa->jenis_kelamin) == $kategori['jenis_kelamin'])
        ...
        <div class="mb-3">
          ...
          <input type="date" class="form-control @error('tgl_lahir') is-invalid @enderror" id="tgl_lahir" name="tgl_lahir" value="{{ old("tgl_lahir", $mahasiswa->tgl_lahir) }}">
          @error('tgl_lahir')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
